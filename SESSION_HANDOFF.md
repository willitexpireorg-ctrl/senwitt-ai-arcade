...
- **Unity pilot (Spatial Memory only):** skills installed (`senwitt-unity-webgl`, Unity Technologies `unity-cli` / `new-unity-project`, plus `unity-csharp-scripting` / `unity-build-pipeline`). Project sources in `unity/SpatialMemoryGrid/`. React host `UnitySpatialMemoryHost.tsx` + `VITE_UNITY_SPATIAL` (default off / HTML fallback). **Unity Editor is not installed on this machine** — need Hub + WebGL module to produce `public/unity/spatial-memory/Build/*`.
